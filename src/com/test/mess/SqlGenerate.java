...
public class SqlGenerate {
    ...
    static String fromTime = "'2017-12-31'";
    static String toTime = "'2018-01-01'";
    ...
    public static String generateAggrSql(){
        return String.format("SELECT  %s 日期,a.account_id 帐号,b.price * 0.01 充值, c.cash_money * 0.01 提现 , d.price * 0.01 购买, e.price * 0.01 退款 , f.balance 余额 FROM (\n"
                                     ...
                                     + "UNION\n"
                                     + "SELECT from_account_id account_id FROM pay_trade WHERE create_time >= %s and create_time <=%s  and type = 2 \n"
                                     + "UNION\n"
                                     + "SELECT to_account_id account_id FROM pay_trade WHERE create_time >= %s and create_time <=%s  and type = 3\n"
                                     + ") a \n"
                                     ...
                                     + "LEFT JOIN (SELECT from_account_id account_id, sum(price) price FROM pay_trade d  WHERE d.create_time >= %s and d.create_time <=%s  and d.type = 2 GROUP BY from_account_id ) d on a.account_id = d.account_id\n"
                                     + "LEFT JOIN (SELECT to_account_id account_id, sum(price) price FROM pay_trade e WHERE e.create_time >= %s and e.create_time <=%s  and e.type = 3 GROUP BY to_account_id) e on a.account_id = e.account_id \n"
